        ...
        <div className="space-y-6">
            <section className="bg-gradient-to-br from-pink-600 to-red-700 rounded-3xl p-8 text-white shadow-lg relative overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <p className="text-pink-100 mt-2">สรุปจำนวนนักเรียนแยกตามชั้น (ทั้งหมด {total} คน)</p>
                </div>
            </section>
            ...
            {loading ? <div className="bg-white rounded-2xl p-8 text-center text-slate-500">กำลังโหลด...</div> : (
                <>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                        {Object.entries(grouped).map(([level, count], i) => (
                            <div key={i} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 text-center hover:shadow-md transition-shadow">
                                ...
                                <div className="text-3xl font-bold text-red-700">{count}</div>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                        <div className="p-4 border-b border-slate-200"><h3 className="font-bold text-slate-800">รายละเอียดแยกตามระดับชั้น</h3></div>
                        <table className="w-full"><thead><tr className="bg-slate-50 border-b border-slate-200">
                            ...
                            <tr key={i} className="border-b border-slate-100 hover:bg-slate-50">
                                ...
                                <td className="px-6 py-3 text-sm text-center"><span className="px-3 py-1 rounded-full text-xs font-bold bg-red-50 text-red-700 border border-red-200">{r.total}</span></td>
                            </tr>
                        ))}</tbody></table>
                    </div>
                </>
